...
TASK_FILE = REPO_ROOT / "codex/tasks/058-phase8g-export-integrity-verifier.md"
DESIGN_DOC = REPO_ROOT / "docs/PHASE8G_EXPORT_INTEGRITY_VERIFIER.md"
...
NEW_PHASE8G_FILES = (TASK_FILE, DESIGN_DOC, VERIFIER_SCRIPT, RUNNER_SCRIPT)

TEST_INPUT_ROOT = REPO_ROOT / "tmp/phase8g-test-input"
...
def work_dir():
    ...
    report_dir_abs = REPO_ROOT / f"tmp/phase8g-export-integrity/pytest-{unique}"
...
def _build_export_fixture(base_dir: Path):
...
def test_phase8g_required_files_exist() -> None:
    for path in (TASK_FILE, DESIGN_DOC, VERIFIER_SCRIPT, RUNNER_SCRIPT, THIS_TEST):
        assert path.is_file(), f"missing Phase 8G file: {path}"


def test_phase8g_status_tokens() -> None:
    assert "phase8g_status: success" in _text(TASK_FILE)
    ...
    assert "durable_audit_store_status: export_integrity_verifier" in text
    assert "signing_implementation_status: not_implemented" in text


# ── B. Scope safety ──────────────────────────────────────────────────────────

def test_phase8g_design_doc_scope_tokens() -> None:
...
def test_missing_manifest_produces_empty_report(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    missing_path = base_dir / "does-not-exist.json"
    result = _run_verifier(
        "--manifest-path", missing_path.relative_to(REPO_ROOT).as_posix(), "--report-dir", report_dir_rel
    )
    assert result.returncode == 0, result.stderr

    json_path = report_dir_abs / "export-integrity-verification.json"
    md_path = report_dir_abs / "export-integrity-verification.md"
    ...
    assert report["hash_only_verification"] is True


# ── E. Valid export behavior ──────────────────────────────────────────────────

def test_valid_export_verifies_clean(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    manifest_path, manifest, evidence_dir = _build_export_fixture(base_dir)
    original_bytes = {p.name: p.read_bytes() for p in evidence_dir.iterdir()}

    result = _run_verifier(
        "--manifest-path", _rel(manifest_path), "--report-dir", report_dir_rel
    )
    assert result.returncode == 0, result.stderr
    ...
    assert report["copied_evidence_count"] == 5
    assert report["issue_count"] == 0
    for entry in report["evidence_results"]:
        assert entry["hash_status"] == "match", entry
        assert entry["size_status"] == "match", entry
    for entry in report["copied_evidence_results"]:
        assert entry["hash_status"] == "match", entry
    assert report["computed_bundle_hash"]
    assert (report_dir_abs / "export-integrity-verification.md").is_file()

    for p in evidence_dir.iterdir():
        assert p.read_bytes() == original_bytes[p.name], f"evidence mutated: {p.name}"
    assert manifest_path.read_text(encoding="utf-8") == json.dumps(manifest, indent=2, sort_keys=True) + "\n"


# ── F. Manifest hash behavior ─────────────────────────────────────────────────

def test_manifest_hash_match(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    ...
    assert result.returncode == 0, result.stderr
    report = json.loads((report_dir_abs / "export-integrity-verification.json").read_text(encoding="utf-8"))
    assert report["manifest_hash_status"] == "match"
    assert report["verification_status"] == "valid"


def test_manifest_hash_mismatch(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    manifest_path, manifest, _ = _build_export_fixture(base_dir)
    ...
    assert result.returncode == 0, result.stderr
    report = json.loads((report_dir_abs / "export-integrity-verification.json").read_text(encoding="utf-8"))
    assert report["manifest_hash_status"] == "mismatch"
    assert report["verification_status"] == "warning"
    assert any(i["issue_type"] == "manifest_hash_mismatch" for i in report["issues"])


# ── G. Bundle hash behavior ────────────────────────────────────────────────────

def test_bundle_hash_computed_only_when_absent(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    manifest_path, manifest, _ = _build_export_fixture(base_dir)
    result = _run_verifier("--manifest-path", _rel(manifest_path), "--report-dir", report_dir_rel)
    assert result.returncode == 0, result.stderr
    report = json.loads((report_dir_abs / "export-integrity-verification.json").read_text(encoding="utf-8"))
    assert report["bundle_hash_status"] == "computed_only"
    assert report["manifest_bundle_hash"] is None
    assert report["computed_bundle_hash"]


def test_bundle_hash_match(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    manifest_path, manifest, _ = _build_export_fixture(base_dir)

    # First run to learn the computed_bundle_hash, then embed it and re-run.
    first = _run_verifier("--manifest-path", _rel(manifest_path), "--report-dir", report_dir_rel)
    assert first.returncode == 0
    first_report = json.loads((report_dir_abs / "export-integrity-verification.json").read_text(encoding="utf-8"))
    manifest["bundle_hash"] = first_report["computed_bundle_hash"]
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    result = _run_verifier("--manifest-path", _rel(manifest_path), "--report-dir", report_dir_rel)
    assert result.returncode == 0, result.stderr
    report = json.loads((report_dir_abs / "export-integrity-verification.json").read_text(encoding="utf-8"))
    assert report["bundle_hash_status"] == "match"
    assert report["verification_status"] == "valid"


def test_bundle_hash_mismatch(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    manifest_path, manifest, _ = _build_export_fixture(base_dir)
    ...
    assert result.returncode == 0, result.stderr
    report = json.loads((report_dir_abs / "export-integrity-verification.json").read_text(encoding="utf-8"))
    assert report["bundle_hash_status"] == "mismatch"
    assert report["verification_status"] == "warning"
    assert any(i["issue_type"] == "bundle_hash_mismatch" for i in report["issues"])


# ── H. Evidence mismatch behavior ─────────────────────────────────────────────

def test_changed_evidence_bytes_cause_hash_mismatch(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    manifest_path, manifest, evidence_dir = _build_export_fixture(base_dir)
    target = evidence_dir / "audit-records.jsonl"
...
def test_changed_evidence_size_causes_size_mismatch(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    manifest_path, manifest, evidence_dir = _build_export_fixture(base_dir)
    # Corrupt only the manifest's recorded size (not the file), so hash mismatch and
    # size mismatch can be exercised independently on the source_evidence entry.
    for entry in manifest["source_evidence"]:
        if entry["label"] == "audit_records_jsonl":
            entry["size_bytes"] = entry["size_bytes"] + 1
            entry["sha256"] = None  # avoid tripping hash comparison for this check
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    result = _run_verifier("--manifest-path", _rel(manifest_path), "--report-dir", report_dir_rel)
    assert result.returncode == 0, result.stderr
    report = json.loads((report_dir_abs / "export-integrity-verification.json").read_text(encoding="utf-8"))
    assert any(i["issue_type"] == "size_mismatch" for i in report["issues"])


def test_missing_evidence_file_causes_issue(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    manifest_path, manifest, evidence_dir = _build_export_fixture(base_dir)
    ...
    report = json.loads((report_dir_abs / "export-integrity-verification.json").read_text(encoding="utf-8"))
    assert report["verification_status"] == "warning"
    assert any(i["issue_type"] in ("missing_evidence_file", "missing_copied_evidence_file") for i in report["issues"])


def test_disallowed_evidence_path_causes_issue(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    manifest_path, manifest, _ = _build_export_fixture(base_dir)
    for entry in manifest["source_evidence"]:
        if entry["label"] == "verification_json":
            entry["path"] = "vault/tampered.json"
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    report = json.loads((report_dir_abs / "export-integrity-verification.json").read_text(encoding="utf-8"))
    assert report["verification_status"] == "warning"
    assert any(i["issue_type"] == "disallowed_evidence_path" for i in report["issues"])


# ── I. Copied evidence behavior ───────────────────────────────────────────────

def test_copied_evidence_hash_verified(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    manifest_path, manifest, _ = _build_export_fixture(base_dir)
    result = _run_verifier("--manifest-path", _rel(manifest_path), "--report-dir", report_dir_rel)
    assert result.returncode == 0, result.stderr
    report = json.loads((report_dir_abs / "export-integrity-verification.json").read_text(encoding="utf-8"))
    assert len(report["copied_evidence_results"]) == 5
    assert all(entry["hash_status"] == "match" for entry in report["copied_evidence_results"])


def test_copied_evidence_hash_mismatch_produces_issue(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    manifest_path, manifest, evidence_dir = _build_export_fixture(base_dir)
    for entry in manifest["copied_files"]:
        if entry["label"] == "query_json":
            entry["sha256"] = "0" * 64
    for entry in manifest["source_evidence"]:
        if entry["label"] == "query_json":
            entry["sha256"] = "0" * 64
    ...
    assert result.returncode == 0, result.stderr
    report = json.loads((report_dir_abs / "export-integrity-verification.json").read_text(encoding="utf-8"))
    assert any(i["issue_type"] == "copied_evidence_hash_mismatch" for i in report["issues"])


def test_copied_to_outside_evidence_dir_produces_issue(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    manifest_path, manifest, _ = _build_export_fixture(base_dir)
    outside = base_dir / "outside-copy.json"
    outside.write_text("{}", encoding="utf-8")
    for entry in manifest["copied_files"]:
        if entry["label"] == "query_json":
            entry["dest_path"] = _rel(outside)
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    result = _run_verifier("--manifest-path", _rel(manifest_path), "--report-dir", report_dir_rel)
    assert result.returncode == 0, result.stderr
    report = json.loads((report_dir_abs / "export-integrity-verification.json").read_text(encoding="utf-8"))
    assert any(i["issue_type"] == "disallowed_copied_evidence_path" for i in report["issues"])


# ── J. Invalid manifest behavior ──────────────────────────────────────────────

def test_invalid_json_manifest_exits_nonzero(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
...
def test_json_array_manifest_exits_nonzero(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
...
def test_reject_manifest_path_under_rejected_roots(root, work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
    marker = REPO_ROOT / root / f"_phase8g_pytest_{uuid.uuid4().hex[:8]}.json"
...
def test_reject_symlinked_manifest(work_dir) -> None:
    base_dir, report_dir_abs, report_dir_rel = work_dir
...
def test_roadmap_references_phase8g() -> None:
    text = _text(ROADMAP)
    assert "Phase 8G" in text
    assert "Export Integrity Verifier Prototype" in text
    for token in ("Phase 5", "read-only", "manual-approved"):
        assert token in text, f"ROADMAP dropped token: {token}"


def test_project_state_references_phase8g() -> None:
    text = _text(PROJECT_STATE)
    assert "docs/PHASE8G_EXPORT_INTEGRITY_VERIFIER.md" in text
    for token in (
...
def test_phase8f_doc_references_phase8g() -> None:
    assert "Phase 8G" in _text(PHASE8F_DOC)


def test_phase8e_doc_references_phase8g() -> None:
    assert "Phase 8G" in _text(PHASE8E_DOC)
...
# ── M. Static safety for new Phase 8G files only ─────────────────────────────

def test_new_phase8g_files_static_safety() -> None:
    banned = (
        "APPROVE_PROMOTE=true",
        "APPROVE_DECISION=true",
        "APPROVE_FINALIZE=true",
