...
def test_phase8k_no_signing_key_or_cert_files_added() -> None:
    ...
    assert not sign_hits, f"Phase 8K must not add signing scripts: {sign_hits}"
    for pattern in ("*.pem", "*.key", "*.crt", "*.pub", "*.p12", "*.pfx"):
        assert not any(scripts_dir.glob(pattern)), f"Phase 8K must not add key/cert files matching {pattern}"
