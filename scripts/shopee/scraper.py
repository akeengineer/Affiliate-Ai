...
def run_scraper(config: dict[str, Any], niche_filter: str | None = None) -> dict[str, Any]:
    """Run the full scraper using browser network-response interception.

    Shopee rejects direct requests to ``/api/v4/search/search_items`` even
    when session cookies are supplied. The search page itself can make that
    request, so this runner captures and parses the response generated by a
    ...
    def intercept_keyword(
        page: Any,
        keyword: str,
        max_products: int,
    ) -> list[dict[str, Any]]:
        """Navigate to one search page and capture its search_items JSON."""
        captured_items: list[dict[str, Any]] = []
        response_received = False
        response_failure: str | None = None

        def handle_response(response: Any) -> None:
            nonlocal response_received, response_failure
            if "search_items" not in response.url:
                return
            ...
            try:
                ...
                response_failure = f"could not parse search_items JSON: {exc}"
                return
            ...
            if (
                ...
                response_failure = f"Shopee search error: {payload['error']}"

        search_url = (
            f"{config['base_url'].rstrip('/')}/search?keyword={quote(keyword)}"
        )
        page.on("response", handle_response)
        try:
            page.goto(search_url, wait_until="domcontentloaded", timeout=timeout_ms)
            deadline = time.monotonic() + scraping_config["timeout_seconds"]
            while not response_received and time.monotonic() < deadline:
                page.wait_for_timeout(250)
        finally:
            page.remove_listener("response", handle_response)

        if not response_received:
            raise TimeoutError("timed out waiting for a search_items response")
        if not captured_items:
            raise RuntimeError(
                response_failure or "search_items response contained no products"
            )

        products: list[dict[str, Any]] = []
        seen_urls: set[str] = set()
        for item in captured_items:
            product = _extract_product_from_item(item, config["base_url"])
            if not product:
                continue
    ...
    def scrape_keyword_with_retry(
        ...
        """Retry one intercepted browser search with exponential backoff."""
        for attempt in range(max_retries):
            try:
                return intercept_keyword(page, keyword, max_products)
            except Exception as exc:
                if attempt == max_retries - 1:
                    print(
                        f"[ERROR] Failed after {max_retries} attempts for "
                        f"'{keyword}': {exc}",
                        file=sys.stderr,
                    )
                    return []
                wait = (backoff ** attempt) * get_delay(config)
                print(
                    f"[RETRY] Attempt {attempt + 1} failed for '{keyword}', "
                    f"waiting {wait:.1f}s",
                    file=sys.stderr,
                )
                time.sleep(wait)
    ...
    with Camoufox(
        ...
        for niche in niches:
            ...
            for keyword in niche.get("keywords", []):
                keyword_products = scrape_keyword_with_retry(page, keyword, max_products)
            ...
            filtered_products = apply_filters(products, config)
            results["niches"][niche_name] = filtered_products
            print(
                f"[INFO] Found {len(filtered_products)} products for '{niche_name}'",
                file=sys.stderr,
            )
