#!/usr/bin/env bash
# Phase 8D Audit Store Query CLI wrapper.
#
# Read-only query CLI over the Phase 8B local append-only JSONL audit store.
# Never appends to or otherwise mutates the source JSONL, never calls the
# Phase 7D wrapper or an approval primitive, never calls the Phase 8B ingest
# writer, never calls the Phase 8C verifier automatically, never writes the
# vault, and never calls a backend/API/database or external service.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
cd "$REPO_ROOT"

QUERY_SCRIPT="$REPO_ROOT/scripts/dev/query_phase8d_audit_store.py"

fail() {
  echo "$1" >&2
  exit 1
}

# ── Guardrails (read-only; reject approval and automation flags) ─────────────
[ "${ENABLE_AUTOPUBLISH:-false}" != "true" ] || fail "ENABLE_AUTOPUBLISH=true is not allowed"
[ "${ENABLE_OPENAI_API_DIRECT:-false}" != "true" ] || fail "ENABLE_OPENAI_API_DIRECT=true is not allowed"
[ "${APPROVE_PROMOTE:-false}" != "true" ] || fail "APPROVE_PROMOTE is not allowed in this read-only command"
[ "${APPROVE_DECISION:-false}" != "true" ] || fail "APPROVE_DECISION is not allowed in this read-only command"
[ "${APPROVE_FINALIZE:-false}" != "true" ] || fail "APPROVE_FINALIZE is not allowed in this read-only command"

[ -f "$QUERY_SCRIPT" ] || fail "Missing query script: $QUERY_SCRIPT"

# ── Select Python: .venv -> python3 -> python ────────────────────────────────
if [ -x "$REPO_ROOT/.venv/bin/python" ]; then
  PYTHON_BIN="$REPO_ROOT/.venv/bin/python"
elif command -v python3 >/dev/null 2>&1; then
  PYTHON_BIN="$(command -v python3)"
else
  PYTHON_BIN="$(command -v python)"
fi

exec "$PYTHON_BIN" "$QUERY_SCRIPT" "$@"
