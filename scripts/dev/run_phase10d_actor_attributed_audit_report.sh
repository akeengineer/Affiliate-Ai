#!/usr/bin/env bash
# Phase 10D actor-attributed audit report wrapper.
#
# Local-only derived actor-attributed audit report prototype. Reads one local
# manifest, validates safe evidence/context references, hashes present files,
# extracts safe summary fields from optional JSON context files, treats safe
...
# tmp/phase10d-actor-attributed-audit-report/. It never calls the Phase 7D
# wrapper, approval primitives, or any Phase 8/9/10C runtime script. It is not
...
BUILD_SCRIPT="$REPO_ROOT/scripts/dev/build_phase10d_actor_attributed_audit_report.py"
...
[ -f "$BUILD_SCRIPT" ] || fail "Missing Phase 10D build script: $BUILD_SCRIPT"
